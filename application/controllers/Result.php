...
class Result extends CI_Controller {
	...
	if($val=='0'){
	 ...
	 $data['percentile'] = $this->result_model->get_percentile($data['result']['quid'], $data['result']['uid'], $data['result']['score_obtained']);
	  ...
	  $Quiz_name=strtolower($data['result']['quiz_name']);
	//Changes For personality Report
	  if(strpos($Quiz_name,"personality") !== false){
		
		
		$this->load->view('header',$data);
		if($this->session->userdata('logged_in')){
		$this->load->view('view_psychometric',$data);
		}else{
