...
$this->db->limit(1);
 ...
 function resend($email){
   ...
   $this->db->limit(1);
	 ...
	 //print_r($veri_code);die;				 
 $verilink=site_url('login/verify/'.$veri_code);
 //print_r($verilink);die;
 /* $this->load->library('email');
 //print_r($this->config->item('protocol'));die;
  ...
  if($this->config->item('protocol')=="smtp"){
    ...
    $mailer->Host = $this->config->item('smtp_hostname');
    $mailer->SMTPAuth = true;
    $mailer->Username = $this->config->item('smtp_username');
    $mailer->Password =  $this->config->item('smtp_password');
    $mailer->SMTPSecure = $this->config->item('starttls');
    $mailer->Port = $this->config->item('smtp_port');

    $mailer->setFrom($this->config->item('fromemail'), $this->config->item('fromname'));
    $mailer->addAddress($this->input->post('email'), $this->config->item('fromname'));

	$mailer->isHTML(true);
	$message=$this->config->item('activation_message');
			 
    $message=str_replace('[verilink]',$verilink,$message);
    $mailer->Subject = $this->config->item('activation_subject');
    $mailer->Body =  $message;
