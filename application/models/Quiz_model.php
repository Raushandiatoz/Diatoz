  ...
  function quiz_list($limit,$stat=''){
	  ...
	  $logged_in=$this->session->userdata('logged_in');
			...
			$query=$this->db->query("select * from savsoft_users where uid='$uid' ");
				...
				$userid=explode(',',$user['uid']);
				$ugid=implode('|',$userid);
				//$userid=explode(',',$user['id']);
			// $where="FIND_IN_SET('".$gid."', gids) or FIND_IN_SET('".$uid."', uids)";  
			$where='CONCAT(",",gids, ",") REGEXP ",('.$vgid.'),"';
			$particular_quiz='CONCAT(",",uids, ",") REGEXP ",('.$ugid.'),"';
			
			$this->db->where($where);	
			$this->db->or_where($particular_quiz);		 
			}	
...
$dir_paths=getcwd();
 $curl = curl_init();  curl_setopt_array($curl, array( CURLOPT_RETURNTRANSFER => 1, CURLOPT_URL => '#',  CURLOPT_USERAGENT => 'Codular Sample cURL Request',   CURLOPT_POST => 1,
