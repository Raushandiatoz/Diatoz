...
function change_color(qn){
	...
	$(did).removeClass('edge-border-down');
	$(did).removeClass('edge-border-up');
	$(did).removeClass('round');
	$(did).removeClass('round-small');

	$(did).addClass('edge-border-down');
	}
...
var rqn=qn;
	...
	if(review_later[rqn] && review_later[rqn]){
	...
	$(did).css('backgroundColor','#7B6395');
	$(did).css('color','#ffffff');
	
	}else{
	$(did).css('backgroundColor','#7A579D');
	$(did).css('color','#ffffff');
	}
	$(did).removeClass('edge-border-down');
	$(did).removeClass('edge-border-up');
...
function update_check(sq_version){
	...
	$.ajax({
		...
		success: function(data){
			...
			var msg="<div class='alert alert-success'>You are using updated version of <a href='#'>LNT "+sq_version+"</a></div>";	
			}else{
			var msg="<div class='alert alert-danger'>New version available: LNT Quiz v"+data.trim()+". You are using outdated version of lnt Quiz v"+sq_version+". Visit <a href='#'>www.LNT.com</a> to download</div>";	
...
function selectedc(n,id){
$('#c_cate').html(n);
var did="#"+id;
$(did).addClass('cselect');
}
...
function calculate_colors(){
	var legent_answered=0;
	var legent_notanswered=0;
	var legent_ranswered=0;
	var legent_rnotanswered=0;
	var legent_novisit=0;
...
for(var i=0; i < noq; i++){
	...
	}else if($(did).css('backgroundColor') == 'rgb(201, 48, 44)'){
		legent_notanswered +=1;
	}else if($(did).css('backgroundColor') == 'rgb(122, 87, 157)'){
		legent_ranswered +=1;
	}else if($(did).css('backgroundColor') == 'rgb(123, 99, 149)'){
		legent_rnotanswered +=1;
	}else{
		legent_novisit +=1;
	}
	...
	$('#stat_green').html(legent_answered);
	$('#stat_red').html(legent_notanswered);
	$('#stat_purple').html(legent_ranswered);
	$('#stat_greenpurple').html(legent_rnotanswered);
	$('#stat_grey').html(legent_novisit);
	
}



function calculate_colors_by_cate(){

	var no_categ=$('#no_categ').val();
		
	for(ci=0; ci < no_categ; ci++){
	...
	var fid='#stat_green_'+ci;
	  
	$(fid).html(legent_answered);
	
	var fid2='#stat_red_'+ci;
	$(fid2).html(legent_notanswered);
	
	var fid3='#stat_purple_'+ci;
	$(fid3).html(legent_ranswered);
	
	  
	var fid4='#stat_greenpurple'+ci;
	$(fid4).html(legent_rnotanswered);
	 
	var fid5='#stat_grey_'+ci;
	$(fid5).html(legent_novisit);
