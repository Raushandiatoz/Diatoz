...
function update_check(sq_version){
	...
	$.ajax({
		 type: "POST",
		 data : formData,
			url: "http://update.lnt.com/",
		success: function(data){
			if(data.trim() <=sq_version){
			var msg="<div class='alert alert-success'>You are using updated version of <a href='http://lnt.com'>lnt Quiz "+sq_version+"</a></div>";	
			}else{
			var msg="<div class='alert alert-danger'>New version available: lnt Quiz v"+data.trim()+". You are using outdated version of lnt Quiz v"+sq_version+". Visit <a href='http://savsoftquiz.com'>www.savsoftquiz.com</a> to download</div>";	
